...
class MidiIO:
    ...
    def send_midi(self, msg):
        ...
        if name == "Launchpad MK2":
            num = -1
            if msg[0]: # working with grid
                num = 10 * (8-msg[1]) + msg[2]+1
            else:
                if msg[1]: # working with side bar
                    num = (8-msg[2])*10 + 9
                else: # working with top bar
                    num = 104 + msg[2] 
            self.midiout.send_message( [0xf0] + [ 0, 32, 41, 2, 24, 11, num] + [msg[3][i]//4 for i in range(0, 3)] + [0xf7] )
            ...
            if msg[0]: # working with grid
                ...
                if msg[1]: # working with side bar
                    ...
                    num = 91 + msg[2]
            self.midiout.send_message( [0xf0] + [ 0, 32, 41, 2, 13, 3, 3, num] + [msg[3][i]//2 for i in range(0, 3)] + [0xf7] )


    """
    process_midi:
    - (input) name is which launchpad
